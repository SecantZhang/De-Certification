...
class Smooth(object):
    ...
    def certify(self, x: torch.tensor, n0: int, n: int, alpha: float, batch_size: int) -> (int, float):
        ...
        if pABar < 0.5:
            ...
            # print(f"cAHat: {cAHat} - {type(cAHat)}, radius: {radius} - {type(radius)}")
            return cAHat, radius
        ...
        with torch.no_grad():
            ...
            for _ in range(ceil(num / batch_size)):
                ...
                if len(x.shape) == 3: 
                    ...
                    batch = x_squeezed.repeat((this_batch_size, 1, 1, 1))
                # print(batch.shape)
                # exit()
                noise = torch.randn_like(batch) * self.sigma
                predictions = self.base_classifier.predict(batch.float() + noise.float()).argmax(1)
